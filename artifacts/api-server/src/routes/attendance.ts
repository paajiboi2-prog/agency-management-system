...
router.post("/attendance/check-in", requireAuth, async (req, res) => {
  try {
    ...
    res.json({ ...record, userName: user.name, checkInAt: record.checkInAt.toISOString(), checkOutAt: null });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: "Internal server error" });
  }
...
router.post("/attendance/check-out", requireAuth, async (req, res) => {
  try {
    ...
    res.json({ ...updated, userName: user.name, checkInAt: updated.checkInAt.toISOString(), checkOutAt: updated.checkOutAt?.toISOString() ?? null });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: "Internal server error" });
  }
